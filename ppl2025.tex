...
遅延モナドを構成する関手Delayは、(A:Type) $\rightarrow$ (\text{cofixpoint} of X = A + X )
という型を持つ関数である。
...
\end{minted}
さて、遅延モナドに付随するオペレーターとして、繰り返し処理を行う\coqin{while}を定義する。
\coqin{while}オペレーターはcomplete elgot monadの$\dagger$オペレーターに相当する。\\
...
CoFixpoint while {A B} (body: A -> M (B + A)) : A -> M B :=
  fun a => (body a) >>= (fun ab => match ab with
                                   ...
                                   |inl b => DNow b end).
\end{minted}
また、\coqin{while}オペレーターを用いて不動点オペレーターの定義が行えることが知られている。
\cite{recursion_from_iteration}では、継続を用いて、\coqin{while}オペレーターから
不動点オペレーターを定義している。
...
まず、遅延モナドのオペレイターは、繰り返し処理を行う\coqin{while}である。また、計算の等さを表す関係である、
...
詳細については2.5節で説明する。
\\
表3は\coqin{while}オペレーターに関する規則である。
Complete elgot monadの定義を参考にした。\\
