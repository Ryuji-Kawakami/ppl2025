...
CoFixpoint while {A B} (body: A -> M (B + A)) : A -> M B :=
  fun a => (body a) >>= (fun ab => match ab with
                                   ...
                                   |inl b => DNow b end).
\end{minted}
また、\coqin{while}オペレーターを用いて再帰関数の定義が行えることが知られている。
\cite{recursion_from_iteration}では、継続を用いて、\coqin{while}オペレーターから不動点オペレーターを定義している。
...
詳細については2.5節で説明する。
\\
表2は\coqin{while}オペレーターに関する規則である。
