...
\inst{$^1$ ○○大学××学科\\
  \texttt{sample@example.ac.jp} \medskip\par
  $^2$ △△研究所\\ \texttt{example@sample.net}}
\begin{document}
\maketitle
\begin{abstract}
...
\begin{table}
  ...
  \begin{tabular}{|l|l|}
    ...
    \coqin{bindfwB}      & \coqin{ (forall a, f a |\scalebox{0.8}{$\approx$}| g a) -> }                              \\
                         & \coqin{d >>= f |\scalebox{0.8}{$\approx$}| d >>= g}                                       \\
    \coqin{whilewB}      & \coqin{ (forall a, f a |\scalebox{0.8}{$\approx$}| g a) -> }                              \\
...
このことを示すことができれば、\mintinline{ssr}|mc91 n 101 \approx Ret 91|であることと、
$ k =  90 - m $に関する帰納法により従う。
